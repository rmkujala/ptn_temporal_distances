...
def plot_plain_profile():
    ...
    fig = analyzer.plot_temporal_distance_profile(format_string="%M", plot_journeys=True, lw=3, plot_tdist_stats=True)
    ax = fig.get_axes()[0]
    ax.set_ylim(0, 11)
    #
    ax.set_xlabel("Departure time (min)")
    fig.tight_layout()
    ax.legend(loc="lower center", fancybox=True, ncol=2, shadow=True, prop={'size': 14})
...
def plot_transfer_profile():
    ...
    labels = [
        LabelTimeWithBoardingsCount(departure_time=label[0], arrival_time_target=label[0] + label[1], n_boardings=label[2], first_leg_is_walk=False)
        for label in labels_raw
    ]
    ...
    print(analyzer.mean_n_boardings_along_shortest_paths())

    fig = analyzer.plot_new_transfer_temporal_distance_profile(format_string="%S", duration_divider=1)
    ax = fig.get_axes()[0]
    ax.set_xlabel("Departure time (min)")
